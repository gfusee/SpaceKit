...
@Controller struct LockController {
    ...
    public func unlockFunds() -> BigUint {
        ...
        Message.caller
            .send(egldValue: attributes.lockedEgldAmount)
        
        return attributes.lockedEgldAmount
    }
    
    public func changeTokenLockDuration(newLockDuration: UInt64) {
        assertOwner()
        
        let tokenRoles = Blockchain.getESDTLocalRoles(tokenIdentifier: self.tokenIdentifier)
        
        guard tokenRoles.contains(flag: .nftUpdateAttributes) else {
            smartContractError(message: "Cannot change token attributes")
        }
        ...
        var attributes = self.getTokenAttributes(nonce: payment.nonce)
        attributes.lockDuration = newLockDuration
    }
