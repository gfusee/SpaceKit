...
fileprivate func retrieveManifest(sourcePackagePath: String) throws(CLIError) -> Manifest {
    let sourcePackageAbsolutePath = try AbsolutePath(validating: sourcePackagePath)
    let workspace = try Workspace(forRootPackage: sourcePackageAbsolutePath)
...
func generateWASMPackage(sourcePackagePath: String) throws(CLIError) -> String {
    ...
    let spaceRequirements = try spaceDependency.toConstraintRequirement()
